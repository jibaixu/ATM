...
CKPT_PATH = "/data2/jibaixu/Codes/ATM/results/track_transformer/0303_libero_track_transformer_bs_128_numtrack_128_libero-object_ep1001_1325/model_best.ckpt"
...
def debug_on():
    ...
    batch_size = int(EXP_NAME.split('_')[5])
    num_track_ids = int(EXP_NAME.split('_')[7])
    sys.argv = [
...
@hydra.main(version_base=None, config_path="./conf/train_track_transformer", config_name="libero_track_transformer")
def main(cfg: DictConfig):
    ...
    model = TrackTransformer(**cfg.model_cfg).to(device=device, dtype=torch.bfloat16)
    ...
    val_dataset = ATMPretrainDataset(dataset_dir=cfg.val_dataset, **cfg.dataset_cfg, aug_prob=0., uniform_sample=True)
    ...
    with torch.no_grad():
        for i, (vid, track, vis, task_emb) in enumerate(tqdm(val_dataloader)):
