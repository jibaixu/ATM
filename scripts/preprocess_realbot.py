...
TRAIN_OUTPUT_PATH = BASE_DIR / "episodes_train_test.jsonl"
VAL_OUTPUT_PATH = BASE_DIR / "episodes_val_test.jsonl"

VIDEO_DIR_NAME = "videos"
ACTION_DIR_NAME = "data"
...
def get_episode_prompt(episode_item: Dict) -> str:
...
def main(skip_exist, dataset_idx, view, min_episode):
    ...
    datasets = sorted([d for d in BASE_DIR.iterdir() if d.is_dir() and d.name.endswith("_1_80")])
    if dataset_idx >= 0:
    ...
    for dataset_dir in datasets:
        ...
        with open(episodes_file, 'r', encoding='utf-8') as f:
            for line in tqdm(f, desc="Extracting Tracks"):
                ...
                prompt = get_episode_prompt(item)
                assert prompt in tasks_map, f"Task not found in tasks.jsonl: {prompt}"
                ...
                for video_view in video_views:
                    ...
                    npz_save_dir = dataset_dir / TRACK_DIR_NAME / chunk_dir_name / track_view_name
                    npz_save_dir.mkdir(parents=True, exist_ok=True)
                    npz_save_path = npz_save_dir / f"{ep_filename}.npz"
                    
                    actual_video_path = dataset_dir / VIDEO_DIR_NAME / chunk_dir_name / video_view / f"{ep_filename}.mp4"

                    video_path = f"{dataset_name}/{VIDEO_DIR_NAME}/{chunk_dir_name}/{video_view}/{ep_filename}.mp4"
                    action_path = f"{dataset_name}/{ACTION_DIR_NAME}/{chunk_dir_name}/{ep_filename}.parquet"
                    track_path = f"{dataset_name}/{TRACK_DIR_NAME}/{chunk_dir_name}/{track_view_name}/{ep_filename}.npz"
